...
#[tokio::test]
async fn graph_tools_hidden_from_the_model_when_toggled_off_but_not_when_no_graph_is_attached() {
    let dir = tempdir().unwrap();
    let mut tools = ToolRegistry::new();
    tools.register(Arc::new(FakeMcpTool {
    ...
    // No graph attached (`set_graph` never called) — the tools stay
    // registered but `graph_enabled()` is unconditionally false, so they're
    // filtered out exactly as if the toggle were off.
    let names: Vec<String> = session
        .tools_for_model()
        .into_iter()
        .map(|t| t.name)
        .collect();
    assert!(!names.contains(&"find_definition".to_string()));
    assert!(!names.contains(&"find_references".to_string()));
    assert!(names.contains(&"local_tool".to_string()));
    assert!(
        !session.set_graph_enabled(false).await,
        "no graph to toggle"
    );

    // Attach a real (in-memory) graph handle: the toggle now actually does
    // something, and defaults to visible.
    let graph = forge_graph::GraphHandle::open_in_memory(dir.path())
        .await
        .unwrap();
    session.set_graph(Some(Arc::new(graph)));
    let names: Vec<String> = session
        .tools_for_model()
        .into_iter()
        .map(|t| t.name)
    ...
    assert!(session.set_graph_enabled(false).await);
    let names: Vec<String> = session
        .tools_for_model()
        .into_iter()
        .map(|t| t.name)
    ...
    assert!(
        names.contains(&"local_tool".to_string()),
        "non-graph tools are unaffected by the toggle"
    );
}
