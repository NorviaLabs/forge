//! TypeScript fact extraction. See `rust.rs` for the shared shape every
//! language extractor follows. Uses `forge_syntax`'s own restriction —
//! `LANGUAGE_TYPESCRIPT`, not `LANGUAGE_TSX` — so `.tsx` JSX syntax isn't
//! specifically handled; that's inherited scope, not a new gap (see
//! `forge-syntax::lang::SyntaxLanguage::tree_sitter`).
...
const TYPES_QUERY: &str = include_str!("../queries/typescript/types.scm");
const IMPLEMENTS_QUERY: &str = include_str!("../queries/typescript/implements.scm");
const FUNCTIONS_QUERY: &str = include_str!("../queries/typescript/functions.scm");
const CALLS_QUERY: &str = include_str!("../queries/typescript/calls.scm");
...
impl Extractor for TypeScriptExtractor {
    fn language(&self) -> forge_syntax::SyntaxLanguage {
        forge_syntax::SyntaxLanguage::TypeScript
    }
    ...
    fn extract(&self, source: &str, path: &Path) -> ExtractedFacts {
        ...
        parser
            ...
            .expect("typescript grammar is valid");
...
mod tests {
    ...
    fn extract(source: &str) -> ExtractedFacts {
        TypeScriptExtractor.extract(source, &PathBuf::from("mod.ts"))
    }
    ...
    #[test]
    fn extracts_extends_and_implements_as_implements_edges() {
        let facts = extract(
            "interface Greeter {}\nclass Base {}\nclass Foo extends Base implements Greeter {}\n",
        );
        let mut targets: Vec<_> = facts
            .edges
            .iter()
            ...
            .collect();
        targets.sort();
        assert_eq!(targets, vec!["Base", "Greeter"]);
    }
