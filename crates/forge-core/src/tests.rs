...
struct GhLooksLikeAuthTool;

#[async_trait]
impl forge_tools::Tool for GhLooksLikeAuthTool {
    fn name(&self) -> &str {
        "gh_auth_looking"
    }

    fn description(&self) -> &str {
        "Simulate gh printing a token error after the proxy 403s"
    }
    ...
    async fn call(
        ...
        // What `gh pr create` actually prints when the CONNECT proxy
        // refuses api.github.com — no "Forge Sandbox Denied" substring.
        Ok(ToolOutput::failed_exit(
...
#[tokio::test]
async fn a_gh_token_error_still_prompts_when_the_proxy_refused_the_host() {
    let dir = tempdir().unwrap();
    let model = Arc::new(MockModelClient::script(vec![]));
    let mut tools = ToolRegistry::new();
    tools.register(Arc::new(GhLooksLikeAuthTool));
    let mut session = AgentSession::create(no_gov_cfg(dir.path()), model, tools)
        .await
        .unwrap();
    session.record_denied_egress_host("api.github.com");
    session.append_user_message("open a pr").await.unwrap();
    ...
    let application = session
        .begin_model_response_application(ModelResponse {
            text: String::new(),
            tool_calls: vec![ToolCall {
                id: "call-gh-auth".into(),
                name: "gh_auth_looking".into(),
                arguments: json!({}),
            }],
            usage: None,
            thinking: None,
    ...
    let completed = pending.execute().await;
    let application = session.finish_tool_application(completed).await.unwrap();
    let ModelResponseApplication::Finished(ApplyOutcome::Hitl(response)) = application else {
        panic!("proxy-refused host must become a grant prompt");
    };
    assert!(
        response
            .text
            .contains("awaiting approval to allow that host"),
        "{}",
        response.text
    );
    let payload = session.pending_hitl().expect("should prompt");
    assert_eq!(payload.denied_host.as_deref(), Some("api.github.com"));
    assert!(!payload.sandbox_escalation);
    assert!(session.messages.iter().all(|message| {
        message.tool_call_id.as_deref() != Some("call-gh-auth") || message.role != MessageRole::Tool
    }));
}

#[tokio::test]
async fn a_real_gh_auth_failure_does_not_prompt_when_the_proxy_allowed_the_host() {
    let dir = tempdir().unwrap();
    let model = Arc::new(MockModelClient::script(vec![]));
    let mut tools = ToolRegistry::new();
    tools.register(Arc::new(GhLooksLikeAuthTool));
    ...
    assert!(
        matches!(
            ...
            ModelResponseApplication::Finished(ApplyOutcome::Continue)
                | ModelResponseApplication::Finished(ApplyOutcome::Done(_))
        ),
        "a GitHub 403 after an allowed CONNECT is a real failure, not a grant prompt"
    );
