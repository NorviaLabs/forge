...
impl TuiApp {
    ...
    pub(super) fn sync_question_menu(&mut self) {
        match self.session_runtime.pending_question() {
    ...
    pub(super) fn sync_question_focus(&mut self) {
        let Some(payload) = self.session_runtime.pending_question() else {
            return;
        };
        if self.question_session.menu.call_id.as_deref() != Some(payload.call_id.as_str()) {
    ...
    pub(super) fn question_presentation(&self) -> Option<QuestionPendingPresentation> {
        let payload = self.session_runtime.pending_question()?;
    ...
    pub(super) async fn handle_question_menu_key(
        ...
        if self.session_runtime.pending_question().is_none() {
            return Ok(false);
        }
        ...
        let Some(payload) = self.session_runtime.pending_question().cloned() else {
            return Ok(false);
        };
    ...
    fn queue_question_submit(&mut self, submit: QuestionSubmit) {
        ...
        if let Some(payload) = self.session_runtime.pending_question() {
