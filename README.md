...
```bash
tar -xzf forge-v0.1.0-alpha.3-<target>.tar.gz
mkdir -p ~/.local/bin
install -m 755 forge-v0.1.0-alpha.3-<target>/forge ~/.local/bin/forge
forge --version
```
...
2. Create an annotated tag matching `v<version>-alpha.<number>`: `git tag -a v0.1.0-alpha.3 -m "Forge 0.1.0 alpha 3"`.
3. Push it: `git push origin v0.1.0-alpha.3`.
