...
```bash
tar -xzf forge-v0.1.0-alpha.4-<target>.tar.gz
mkdir -p ~/.local/bin
install -m 755 forge-v0.1.0-alpha.4-<target>/forge ~/.local/bin/forge
forge --version
```
...
2. Create an annotated tag matching `v<version>-alpha.<number>`: `git tag -a v0.1.0-alpha.4 -m "Forge 0.1.0 alpha 4"`.
3. Push it: `git push origin v0.1.0-alpha.4`.
