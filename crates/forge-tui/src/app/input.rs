...
impl TuiApp {
        ...
        match key.code {
            ...
            KeyCode::Char('s') if key.modifiers.is_empty() => {
                    ...
                    .is_some_and(|session| session.session_id == session_id)
                {
                    self.set_feedback(
                        FeedbackSeverity::Info,
                        "use Esc or Ctrl+C to stop the selected Session",
                    );
                    return Ok(true);
                }
            ...
            KeyCode::Char('c') if key.modifiers.is_empty() => {
                    ...
                    .is_some_and(|session| session.session_id == session_id)
                {
                    self.set_feedback(
                        FeedbackSeverity::Info,
                        "the selected Session is controlled by the composer",
                    );
                    return Ok(true);
                }
    ...
    pub(super) fn take_session_view_state(&mut self) -> SessionViewState {
        ...
        SessionViewState {
            ...
            turn_summaries: std::mem::take(&mut self.turn_summaries),
            turn_summaries_revision: self.turn_summaries_revision,
    ...
    fn install_session_view_state(&mut self, state: SessionViewState) {
        ...
        self.turn_summaries = state.turn_summaries;
        self.turn_summaries_revision = state.turn_summaries_revision;
    ...
    /// repository mode every runtime is actor-owned, while its immutable
    /// snapshot supplies the filesystem root for explorer and repository UI.
    ...
    pub(super) async fn submit_composer_message(&mut self) -> Result<(), TuiError> {
        ...
        let route = self.selected_input_route(&line);
        ...
        match route {
            ...
            input_route::InputRoute::AnswerClarification => {
                if self.selected_pending_question().is_some() {
