...
HTTPS (SSH cannot use the CONNECT proxy), and git-dir writes are allowed
only for git frontends. Ordinary commands still cannot write `.git`.
The sandbox cannot open the OS secret store, so without projection an
HTTPS client prints HTTP 401 / "invalid token" and looks like broken
auth.
