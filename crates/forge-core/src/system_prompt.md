...
- Proactively run tests, lint, and build when that is how the project verifies work. Those commands run confined; a host they need will surface as a network permission prompt rather than as a command approval.
- When working on test-related tasks, such as adding tests, fixing tests, or reproducing a bug to verify behavior, run the tests. Use your judgement to decide whether this is a test-related task.
