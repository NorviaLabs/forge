...
- Repository-local sessions, offloaded context, and progress are stored under `.forge/` in the workspace.
...
Remove repository-local Forge state by deleting `.forge/` from the test repository.
