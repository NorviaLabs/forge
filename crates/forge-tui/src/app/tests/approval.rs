...
// ---------------------------------------------------------------------------
// Permission mode is clamped to what the host can enforce.
//
// The invariant: Auto is only ever entered when there is an enforcement floor
// underneath it. `AcceptEdits` frees writes, which is only defensible when a
// sandbox confines what a freed write can reach.
// ---------------------------------------------------------------------------

use forge_governance::PermissionMode;

/// What this host is actually allowed to reach, so the assertions below hold
/// on a developer Mac (sandbox present) and on CI Linux (no sandbox yet).
fn host_ceiling() -> PermissionMode {
    forge_core::permission_ceiling().0
}

#[tokio::test]
async fn requesting_auto_is_capped_to_what_the_host_can_enforce() {
    let (_dir, mut app) = focus_test_app().await;
    let capped = app.set_permission_mode_clamped(PermissionMode::AcceptEdits);

    match host_ceiling() {
        PermissionMode::AcceptEdits => {
            assert_eq!(app.session.permission_mode(), PermissionMode::AcceptEdits);
            assert!(capped.is_none(), "nothing was refused, so nothing to say");
        }
        PermissionMode::Manual => {
            assert_eq!(
                app.session.permission_mode(),
                PermissionMode::Manual,
                "Auto must be unreachable without a sandbox"
            );
            assert!(
                capped.is_some_and(|reason| !reason.is_empty()),
                "a refused request must explain itself"
            );
        }
    }
}

/// Asking to be interrupted more often is always safe, so it is never refused
/// — including on a host that could support Auto.
#[tokio::test]
async fn choosing_manual_is_never_overridden() {
    let (_dir, mut app) = focus_test_app().await;
    let capped = app.set_permission_mode_clamped(PermissionMode::Manual);
    assert_eq!(app.session.permission_mode(), PermissionMode::Manual);
    assert!(capped.is_none(), "narrowing is not a refusal");
}

/// `PermissionMode` defaults to `AcceptEdits`, so a session with no saved
/// state would start in Auto with nothing underneath it. Restoring must clamp
/// even when there is nothing to restore.
#[tokio::test]
async fn a_session_never_starts_above_the_ceiling() {
    let (_dir, mut app) = focus_test_app().await;
    app.load_ui_state();
    assert_eq!(
        app.session.permission_mode(),
        app.session.permission_mode().clamped_to(host_ceiling()),
        "startup mode must already be within the ceiling"
    );
}
