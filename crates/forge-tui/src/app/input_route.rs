...
use forge_types::{HitlDecision, TaskLifecycle, WaitReason};
...
pub(crate) enum InputRoute {
    ...
    QueueFutureTask,
    /// `Waiting` on `WaitReason::Approval` with a recognized y/n-style token.
    ResolveApproval(HitlDecision),
...
/// Parse a short approve/deny token. Deliberately narrow — composer text
/// isn't meant to carry free-form approval semantics; anything else falls
/// through to `RejectStaleResponse` so the operator gets an explicit nudge
/// rather than an silently misrouted message.
pub(crate) fn approval_token(line: &str) -> Option<HitlDecision> {
    match line.trim().to_ascii_lowercase().as_str() {
        "y" | "yes" | "approve" => Some(HitlDecision::Approve),
        "n" | "no" | "deny" => Some(HitlDecision::Deny),
        _ => None,
    }
}
...
pub(crate) fn classify_input(
    ...
    if active.lifecycle == TaskLifecycle::Waiting {
        ...
        return match &active.wait_reason {
            Some(WaitReason::Approval { .. }) => match approval_token(line) {
                Some(decision) => InputRoute::ResolveApproval(decision),
...
mod tests {
    ...
    fn waiting_on_approval_with_yes_resolves_approve() {
        ...
        assert_eq!(
            ...
            InputRoute::ResolveApproval(HitlDecision::Approve)
        );
        assert_eq!(
    ...
    fn waiting_on_approval_with_no_resolves_deny() {
        ...
        assert_eq!(
            ...
            InputRoute::ResolveApproval(HitlDecision::Deny)
        );
    }
    ...
    #[test]
    fn approval_token_is_case_insensitive_and_trims_whitespace() {
        assert_eq!(approval_token("  Yes  "), Some(HitlDecision::Approve));
        assert_eq!(approval_token("DENY"), Some(HitlDecision::Deny));
        assert_eq!(approval_token("maybe"), None);
    }
}
