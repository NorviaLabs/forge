...
impl SourceViewer {
    ...
    ///
    /// Compare contents rather than relying on size and mtime: filesystems
    /// with coarse timestamp resolution can preserve both metadata values
    /// across a rapid external rewrite, which would otherwise lose edits.
