//! Rust fact extraction — the first language implemented (see the plan's
//! rationale: forge's own repo is the dogfood target, `impl Trait for Type`
//! cleanly exercises the `implements` edge, no JSX/ASI wrinkles).
...
const IMPLEMENTS_QUERY: &str = include_str!("../queries/rust/implements.scm");
...
impl Extractor for RustExtractor {
    fn language(&self) -> forge_syntax::SyntaxLanguage {
        forge_syntax::SyntaxLanguage::Rust
    }
...
/// 1-indexed line for human-facing output (`node.start_position()` is
/// 0-indexed).
...
/// Nearest enclosing `function_item`'s name, if any — used as a call site's
/// `src_symbol`. `None` for a call at module scope (rare, but real: const
/// initializers, top-level macro invocations).
...
fn extract_implements(
    ...
    while let Some(m) = matches.next() {
        ...
        if let (Some(trait_name), Some(type_name), Some(node)) = (trait_name, type_name, at_node) {
            facts.edges.push(EdgeFact {
...
mod tests {
    ...
    fn extract(source: &str) -> ExtractedFacts {
        RustExtractor.extract(source, &PathBuf::from("src/lib.rs"))
    }
...
"#,
    ...
    #[test]
    fn extracts_implements_edge_for_simple_impl_trait_for_type() {
        let facts = extract(
            r#"
trait Greet {}
struct Foo;
...
"#,
        ...
        assert_eq!(implements[0].target_name, "Greet");
    }
...
"#,
    ...
    #[test]
    fn extracts_method_call_and_scoped_call_callee_names() {
        let facts = extract(
            r#"
fn caller(x: Foo) {
    x.process();
...
"#,
        ...
        let callees: Vec<_> = facts
            ...
            .collect();
        assert_eq!(callees, vec!["process", "process"]);
    }

    #[test]
    fn extracts_a_simple_use_import() {
        let facts = extract("use std::collections::HashMap;\n");
    ...
    fn extracts_grouped_use_imports() {
        let facts = extract("use std::collections::{HashMap, HashSet};\n");
        let mut imports: Vec<_> = facts
            ...
            .collect();
        imports.sort();
        assert_eq!(imports, vec!["HashMap", "HashSet"]);
    }

    #[test]
    fn file_path_is_carried_verbatim_onto_every_fact() {
        let facts = RustExtractor.extract("fn f() {}", &PathBuf::from("crates/x/src/f.rs"));
        assert_eq!(facts.symbols[0].file, "crates/x/src/f.rs");
    }
}
